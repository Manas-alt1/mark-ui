    ...
    <div style={{ maxWidth: '960px', margin: '0 auto', padding: '64px 24px' }}>
      <motion.div variants={containerVariants} initial="hidden" animate="show">
        <motion.div variants={itemVariants} style={{ marginBottom: '48px' }}>
          ...
          <h1 style={{ fontFamily: 'var(--mark-font-display)', fontSize: 'clamp(36px, 5vw, 48px)', fontWeight: 800, color: 'var(--mark-fg)', margin: '0 0 12px' }}>Toggle</h1>
          <p style={{ fontFamily: 'var(--mark-font-body)', fontSize: 'var(--mark-text-lg)', color: 'var(--mark-fg-muted)', lineHeight: 1.6, maxWidth: '640px', margin: 0 }}>On/off switch. The slide animation uses spring physics for a physical, satisfying feel.</p>
        </motion.div>

        <Section label="Off and On States">
        ...
        <Section label="With Label + Description">
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <Toggle label="Dark mode" description="Toggle between light and dark themes" defaultChecked />
            <Toggle label="Notifications" description="Receive push notifications for updates" />
          </div>
        </Section>
        ...
        <Section label="Animation Details">
          <div style={{ background: 'var(--mark-bg-elevated)', border: '1px solid var(--mark-border)', borderRadius: 'var(--mark-radius-md)', padding: '16px', fontFamily: 'var(--mark-font-code)', fontSize: '13px', color: 'var(--mark-fg)', lineHeight: 1.7 }}>
            <p style={{ margin: '0 0 8px' }}>Thumb: spring · stiffness: 500 · damping: 30 (physical snap)</p>
            <p style={{ margin: 0 }}>Track: CSS transition · --mark-duration-normal · --mark-ease-smooth</p>
          </div>
        </Section>
      </motion.div>
    </div>
